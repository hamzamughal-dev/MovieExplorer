...
function MediaExplorerPage({ type = 'movie', title, searchPlaceholder, discoverText }) {
    ...
    const {
        ...
        error,
        isSearchMode,
    } = useMediaList(type, debouncedQuery);
    ...
    return (
        <div className="min-h-full bg-[#202731] text-white px-6 pt-5 pb-8">
            <div className="max-w-7xl mx-auto px-2 md:px-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                <div>
                    <h1 className="text-[32px] md:text-[38px] font-extrabold bg-gradient-to-r from-[#01b4e4] to-[#90cea1] bg-clip-text text-transparent mb-[2px] w-fit">
                        {hasSearchInput ? 'Search Results' : title}
                    </h1>
                    <p className="text-slate-400 text-[14px] md:text-[15px]">
                        {hasSearchInput ? `Results for "${searchQuery}"` : discoverText}
                    </p>
                </div>
            ...
            {isLoading && !isSearchMode && <Loader text={`Fetching ${type === 'tv' ? 'TV shows' : 'movies'}...`} />}
            {isLoading && isSearchMode && <Loader text={`Searching ${type === 'tv' ? 'TV shows' : 'movies'}...`} />}

            {error && (
                <div className="text-center p-[40px] bg-red-500/10 border border-red-500/30 rounded-[16px] text-red-400 text-[16px]">
                    ⚠️ {error.message || `Something went wrong while fetching ${type === 'tv' ? 'TV shows' : 'movies'}.`}
                </div>
            )}

            {!isLoading && !error && displayItems.length === 0 && (
                <div className="text-center p-[40px] bg-white/5 border border-white/10 rounded-[16px] text-slate-400 text-[16px]">
                    {isSearchMode ? `No ${type === 'tv' ? 'TV shows' : 'movies'} found matching "${searchQuery}".` : `No ${type === 'tv' ? 'TV shows' : 'movies'} found.`}
                </div>
            )}
                ...
                <>
                    ...
                    {isFetchingNextPage && (
                        <div className="mt-8 flex justify-center">
                            <Loader text={`Loading more ${type === 'tv' ? 'TV shows' : 'movies'}...`} className="h-[100px]" />
                        </div>
                    )}
                </>
            )}
...
export default MediaExplorerPage;
